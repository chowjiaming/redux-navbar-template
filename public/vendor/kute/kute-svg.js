...
(function(root, factory) {
  ...
  var g = typeof global !== "undefined" ? global : window,
    K = KUTE, // connect plugin to KUTE object and global
    DOM = K.dom,
    parseProperty = K.parseProperty,
    prepareStart = K.prepareStart,
    getCurrentStyle = K.getCurrentStyle,
    trueColor = K.truC,
    trueDimension = K.truD,
    crossCheck = K.crossCheck,
    number = g.Interpolate.number,
    unit = g.Interpolate.unit,
    color = g.Interpolate.color, // interpolate functions
  ...
  if (isIE && isIE < 9) {
    return;
  } // return if SVG API is not supported

  // here we go with the plugin
  var pathReg = /(m[^(h|v|l)]*|[vhl][^(v|h|l|z)]*)/gim,
    ...
    coords = (g.Interpolate.coords = function(a, b, l, v) {
      ...
      for (var i = 0; i < l; i++) {
        ...
        for (var j = 0; j < 2; j++) {
          ...
          points[i].push(
            (((a[i][j] + (b[i][j] - a[i][j]) * v) * 1000) >> 0) / 1000
          );
        }
      }
      return points;
  ...
  // SVG MORPH
  var getSegments = function(s, e, r) {
      // getSegments returns an array of points based on a sample size morphPrecision
      var s1 = [],
        e1 = [],
        le1 = s.getTotalLength(),
        le2 = e.getTotalLength(),
        ml = Math.max(le1, le2),
        d = r,
        ar = ml / r,
        j = 0,
        sl = ar * r; // sl = sample length
    ...
    getClosestPoint = function(p, t, s) {
      // utility for polygon paths, returns a close point from the original path (length,pointAtLength,smallest); // intervalLength
      var x,
        y,
        a = [],
        l = s.length,
      ...
      pr = !!s[dx - 1] ? dx - 1 : l - 1;
      nx = !!s[dx + 1] ? dx + 1 : 0;
      return Math.abs(s[pr][0] - t.x) < p && Math.abs(s[pr][1] - t.y) < p
        ? s[pr]
        : Math.abs(s[nx][0] - t.x) < p && Math.abs(s[nx][1] - t.y) < p
        ? s[nx]
        : Math.abs(s[dx][0] - t.x) < p && Math.abs(s[dx][1] - t.y) < p
        ? s[dx]
        : [t.x, t.y];
    },
    pathToAbsolute = function(p) {
      // simple utility for polygons | this is still BETA / a work in progress
      var np = p.match(pathReg),
        wp = [],
        l = np.length,
        s,
        c,
    ...
    getOnePath = function(p) {
      return p.split(/z/i).shift() + "z";
    }, // we only tween first path only
    createPath = function(p) {
      // create a <path> when glyph
      var createdPath = document.createElementNS(ns, "path"),
        d = typeof p === "object" ? p.getAttribute("d") : p;
      createdPath.setAttribute("d", d);
      return createdPath;
    },
    forcePath = function(p) {
      // forcePath for glyph elements
      if (p.tagName === "glyph") {
        // perhaps we can also change other SVG tags in the future
        var c = createPath(p);
    ...
    computePathCross = function(s, e) {
      ...
      if (!this._isPolygon) {
        ...
        if (s.length !== e.length) {
          ...
          simulatedSmallerPath = createPath("M" + smallerPath.join("L") + "z");
          len = simulatedSmallerPath.getTotalLength() / largerPathLength;
      ...
      // reverse arrays
      if (this.options.reverseFirstPath) {
        s1.reverse();
      }
      if (this.options.reverseSecondPath) {
        e1.reverse();
      }
  ...
  parseProperty.path = function(o, v) {
    if (!("path" in DOM)) {
      DOM.path = function(l, p, a, b, v) {
        l.setAttribute(
          "d",
          v === 1 ? b.o : "M" + coords(a["d"], b["d"], b["d"].length, v) + "Z"
        );
      };
    }
    return getPath(v);
  ...
  crossCheck.path = function() {
    // unlike other cases, the crossCheck apply to both to() and fromTo() methods
    var p1 = getOnePath(this.valuesStart.path.o),
      p2 = getOnePath(this.valuesEnd.path.o),
      paths;

    // path tween options
    this.options.morphPrecision =
      this.options && "morphPrecision" in this.options
        ? parseInt(this.options.morphPrecision)
  ...
  var percent = function(v, l) {
    ...
    getPolyLength = function(el) {
      ...
      if (points.length > 1) {
        ...
        len +=
          el.tagName === "polygon"
            ? dist(coord(points[0]), coord(points[points.length - 1]))
            : 0;
      }
      return len;
    },
    ...
    getDraw = function(e, v) {
      var l = /path|glyph/.test(e.tagName)
          ? e.getTotalLength()
          : getTotalLength(e),
        start,
        end,
  ...
  parseProperty.draw = function(a, o) {
    // register the draw property
    if (!("draw" in DOM)) {
      DOM.draw = function(l, p, a, b, v) {
        var pathLength = ((a.l * 100) >> 0) / 100,
          start = ((number(a.s, b.s, v) * 100) >> 0) / 100,
          end = ((number(a.e, b.e, v) * 100) >> 0) / 100,
          offset = 0 - start,
          dashOne = end + offset;
  ...
  var parseStringOrigin = function(origin, box) {
      return /[a-zA-Z]/.test(origin) && !/px/.test(origin)
        ...
        : /%/.test(origin)
        ? box.x + (parseFloat(origin) * box.width) / 100
        : parseFloat(origin);
    },
    parseTransformString = function(a) {
    ...
    parseTransformObject = function(v) {
      var svgTransformObject = {},
        bb = this.element.getBBox(),
        cx = bb.x + bb.width / 2,
        cy = bb.y + bb.height / 2, // by default the transformOrigin is "50% 50%" of the shape box
  ...
  parseProperty.svgTransform = function(p, v) {
    ...
    if (!("svgTransform" in DOM)) {
      DOM.svgTransform = function(l, p, a, b, v) {
        ...
        y -= complex ? b.origin[1] : 0; // we start with removing transformOrigin from translation
        x *= scale;
        y *= scale; // we now apply the scale
        y += skewY ? x * Math.tan(skewY * deg) : 0;
        x += skewX ? y * Math.tan(skewX * deg) : 0; // now we apply skews
        ...
        x += complex ? b.origin[0] : 0;
        y += complex ? b.origin[1] : 0; // normalizing ends with the addition of the transformOrigin to the translation
  ...
  prepareStart.svgTransform = function(p, t) {
    var transformObject = {},
      currentTransform = parseTransformString(
        this.element.getAttribute("transform")
      );
    for (var i in t) {
      transformObject[i] =
        i in currentTransform ? currentTransform[i] : i === "scale" ? 1 : 0;
    } // find a value in current attribute value or add a default value
    return transformObject;
  };

  crossCheck.svgTransform = function() {
    // helper function that helps preserve current transform properties into the objects
    if (!this.options.rpr) return; // fix since 1.6.1 for fromTo() method
    var valuesStart = this.valuesStart.svgTransform,
      valuesEnd = this.valuesEnd.svgTransform,
      currentTransform = parseTransformObject.call(
        this,
    ...
    var parentSVG = this.element.ownerSVGElement,
      newTransform = parentSVG.createSVGTransformFromMatrix(
        parentSVG
          ...
          .translate(
            "translate" in valuesStart ? valuesStart.translate[0] : 0,
