...
(function(root, factory) {
  ...
  for (
    var propertyIndex = 0, allCorePropLength = coreProps[length], coreProp;
    ...
    } else if (coreProp === "translate3d") {
      // px
      defaultPropsValues[coreProp] = [0, 0, 0];
    } else if (coreProp === "translate") {
      // px
      defaultPropsValues[coreProp] = [0, 0];
    } else if (coreProp === "rotate" || /X|Y|Z/.test(coreProp)) {
      // deg
      defaultPropsValues[coreProp] = 0;
    } else if (coreProp === "scale" || coreProp === "opacity") {
      // unitless
  ...
  var defaultOptions = {
    ...
    getPrefix = function() {
      ...
      for (var pIndex = 0, pfl = prefixes[length]; pIndex < pfl; pIndex++) {
        if (prefixes[pIndex] + "Transform" in body[style]) {
          thePrefix = prefixes[pIndex];
          break;
        }
      }
      return thePrefix;
    },
    property = function(propertyToPrefix) {
      // returns prefixed property | property
      var prefixRequired = !(propertyToPrefix in body[style]) ? true : false,
        prefix = getPrefix(); // is prefix required for property | prefix
      return prefixRequired
        ? prefix +
            (propertyToPrefix.charAt(0).toUpperCase() +
              propertyToPrefix.slice(1))
        : propertyToPrefix;
    },
    selector = function(el, multi) {
      // a public selector utility
      var requestedElem;
      if (multi) {
        requestedElem =
          el instanceof Object || typeof el === "object"
            ? el
            : document.querySelectorAll(el);
      } else {
        requestedElem =
          typeof el === "object" ? el : document.querySelector(el);
      }
      if (requestedElem === null && el !== "window")
    ...
    trueColor = function(colorString) {
      ...
      if (/rgb|rgba/.test(colorString)) {
        ...
        return { r: fromHex.r, g: fromHex.g, b: fromHex.b };
        ...
        var webColor = g.getComputedStyle(siteHead, null).color;
        webColor = /rgb/.test(webColor)
          ? webColor[replace](/[^\d,]/g, "")[split](",")
          : [0, 0, 0];
    ...
    getInlineStyle = function(el) {
      ...
      var css = el[style].cssText[replace](/\s/g, "")[split](";"),
        transformObject = {}; // the cssText | the resulting transform object
      ...
      for (var i = 0, csl = css[length]; i < csl; i++) {
        if (/transform/i.test(css[i])) {
          ...
          for (var k = 0, tpl = tps[length] - 1; k < tpl; k++) {
            var tpv = tps[k][split]("("),
              tp = tpv[0],
              tv = tpv[1]; // each transform property, the sp is for transform property
            if (transformFunctions[indexOf](tp) !== -1) {
              transformObject[tp] = /translate3d/.test(tp)
                ? tv[split](",")
                : tv;
            }
          }
    ...
    getCurrentStyle = function(elem, propertyName) {
      // get computed style property for element for .to() method
      var styleAttribute = elem[style],
        computedStyle = g.getComputedStyle(elem, null) || elem.currentStyle,
        prefixedProp = property(propertyName), //the computed style | prefixed property
        styleValue =
          styleAttribute[propertyName] &&
          !/auto|initial|none|unset/.test(styleAttribute[propertyName])
            ? styleAttribute[propertyName]
            : computedStyle[prefixedProp];
      if (
        propertyName !== "transform" &&
        (prefixedProp in computedStyle || prefixedProp in styleAttribute)
      ) {
        if (styleValue) {
          if (prefixedProp === "filter") {
            // handle IE8 opacity
            var filterValue = parseInt(
              styleValue[split]("=")[1][replace](")", "")
            );
    ...
    canTouch =
      "ontouchstart" in g || (navigator && navigator.msMaxTouchPoints) || false, // support Touch?
    touchOrWheel = canTouch ? "touchstart" : "mousewheel",
    mouseEnter = "mouseenter", //events to prevent on scroll
    ...
    scrollContainer =
      navigator && /(EDGE|Mac)/i.test(navigator.userAgent) ? body : html,
    ...
    number = (interpolate.number = function(a, b, v) {
      // number1, number2, progress
      a = +a;
      b -= a;
      return a + b * v;
    }),
    unit = (interpolate.unit = function(a, b, u, v) {
      // number1, number2, unit, progress
      a = +a;
      b -= a;
      return a + b * v + u;
    }),
    color = (interpolate.color = function(a, b, v, toHex) {
      ...
      for (c in b) {
        _c[c] =
          c !== "a"
            ? number(a[c], b[c], v) >> 0 || 0
            : a[c] && b[c]
            ? ((number(a[c], b[c], v) * 100) >> 0) / 100
            : null;
      }
      return toHex
        ? rgbToHex(_c.r, _c.g, _c.b)
        : !_c.a
    ...
    translate = (interpolate.translate = function(a, b, u, v) {
      ...
      for (var ax in b) {
        translation[ax] =
          (a[ax] === b[ax]
            ? b[ax]
            : (((a[ax] + (b[ax] - a[ax]) * v) * 1000) >> 0) / 1000) + u;
      }
    ...
    update = function(t) {
      ...
      var elapsed = Math.min(
          (t - this._startTime) / this[options][duration],
          1
        ),
        progress = this[options][easing](elapsed); // calculate progress

      for (var tweenProp in this[valuesEnd]) {
        // render the DOM update
        DOM[tweenProp](
          this[element],
          tweenProp,
      ...
      if (elapsed === 1) {
        if (this[options][repeat] > 0) {
          if (isFinite(this[options][repeat])) {
            this[options][repeat]--;
          }
          ...
          this._startTime =
            this[options][yoyo] && !this.reversed
              ? t + this[options][repeatDelay]
              : t; //set the right time for delay
          return true;
        } else {
    ...
    perspective = function() {
      var el = this[element],
        ops = this[options];
      if (
        ops.perspective !== undefined &&
        transformProperty in this[valuesEnd]
      ) {
        // element perspective
        this[valuesStart][transformProperty]["perspective"] = this[valuesEnd][
          transformProperty
        ]["perspective"];
      }
      ...
      if (
        ops.transformOrigin !== undefined &&
        !("svgTransform" in this[valuesEnd])
      ) {
      ...
      if (ops.parentPerspective !== undefined) {
        el.parentNode[style][property("perspective")] =
          ops.parentPerspective + "px";
      } // parent perspective
      if (ops.parentPerspectiveOrigin !== undefined) {
        el.parentNode[style][property("perspectiveOrigin")] =
          ops.parentPerspectiveOrigin;
    ...
    parseProperty = {
      ...
      transform: function(tweenProp, inputValue) {
        if (!(transformProperty in DOM)) {
          DOM[transformProperty] = function(elem, tweenProp, a, b, v, o) {
            elem[style][tweenProp] =
              (a.perspective || "") +
              ("translate" in a
                ? translate(a.translate, b.translate, "px", v)
                : "") +
        ...
        if (/translate/.test(tweenProp)) {
          if (tweenProp === "translate3d") {
            var t3d = inputValue[split](","),
              t3d0 = trueDimension(t3d[0]),
              t3d1 = trueDimension(t3d[1], (t3d2 = trueDimension(t3d[2])));
            return {
              translateX:
                t3d0.u === "%"
                  ? (t3d0.v * this[element][offsetWidth]) / 100
                  : t3d0.v,
              translateY:
                t3d1.u === "%"
                  ? (t3d1.v * this[element][offsetHeight]) / 100
                  : t3d1.v,
              translateZ:
                t3d2.u === "%"
                  ? (t3d2.v *
                    ...
                    200
                  : t3d2.v // to be changed with something like element and/or parent perspective
            };
          } else if (/^translate(?:[XYZ])$/.test(tweenProp)) {
            var t1d = trueDimension(inputValue),
              percentOffset = /X/.test(tweenProp)
                ? this[element][offsetWidth] / 100
                : /Y/.test(tweenProp)
                ? this[element][offsetHeight] / 100
            ...
            var tv =
              ...
              t2d1 = tv[length] ? trueDimension(tv[1]) : { v: 0, u: "px" };
            if (tv instanceof Array) {
              (t2d.x =
                t2d0.u === "%"
                  ? (t2d0.v * this[element][offsetWidth]) / 100
                  : t2d0.v),
                (t2d.y =
                  t2d1.u === "%"
                    ? (t2d1.v * this[element][offsetHeight]) / 100
                    : t2d1.v);
            } else {
              t2dv = trueDimension(tv);
              (t2d.x =
                t2dv.u === "%"
                  ? (t2dv.v * this[element][offsetWidth]) / 100
                  : t2dv.v),
                (t2d.y = 0);
            }
          ...
          if (/^rotate(?:[XYZ])$|skew(?:[XY])$/.test(tweenProp)) {
            ...
            var r2d = {},
              r2dv = trueDimension(inputValue, true);
      ...
      unitless: function(tweenProp, inputValue) {
        // scroll | opacity
      ...
      colors: function(tweenProp, inputValue) {
        // colors
        if (!(tweenProp in DOM)) {
    ...
    preparePropertiesObject = function(obj, fn) {
      ...
      var propertiesObject =
          fn === "start" ? this[valuesStart] : this[valuesEnd],
        skewObject = {},
        rotateObject = {},
        translateObject = {},
        transformObject = {};
      ...
      for (var x in obj) {
        if (transformFunctions[indexOf](x) !== -1) {
          ...
          if (/^translate(?:[XYZ]|3d)$/.test(x)) {
            ...
            for (var fnIndex = 0; fnIndex < 3; fnIndex++) {
              ...
              if (/3d/.test(x)) {
                ...
                ] = parseProperty.transform.call(
                  this,
                  "translate" + translateAxis,
                  obj[x][fnIndex]
                );
              } else {
                translateObject["translate" + translateAxis] =
                  "translate" + translateAxis in obj
                    ? parseProperty.transform.call(
                        this,
                        "translate" + translateAxis,
                        obj["translate" + translateAxis]
                      )
                    : 0;
              }
            }
            ...
            var objectName = /rotate/.test(x) ? "rotate" : "skew",
              rotationOrSkew =
                objectName === "rotate" ? rotateObject : skewObject;
    ...
    close = function() {
      ...
      if (this[options][yoyo] && this.reversed === true) {
        reverse.call(this);
        this.reversed = false;
      }
      this[playing] = false;
    ...
    preventScroll = function(eventObj) {
    ...
    getStartValues = function() {
      ...
      for (var tweenProperty in this[valuesStart]) {
        if (transformFunctions[indexOf](tweenProperty) !== -1) {
          ...
          if (
            /translate/.test(tweenProperty) &&
            tweenProperty !== "translate"
          ) {
            startValues["translate3d"] =
              currentStyle["translate3d"] || defaultPropsValues[tweenProperty];
          } else if (r2d) {
            // 2d transforms
            startValues[tweenProperty] =
              currentStyle[tweenProperty] || defaultPropsValues[tweenProperty];
          } else if (!r2d && /rotate|skew/.test(tweenProperty)) {
            // all angles
            for (var degIndex = 0; degIndex < 2; degIndex++) {
              for (var axisIndex = 0; axisIndex < 3; axisIndex++) {
                var s = degreeProps[degIndex] + startAxis[axisIndex];
                if (
                  transformFunctions[indexOf](s) !== -1 &&
                  s in this[valuesStart]
                ) {
                  startValues[s] = currentStyle[s] || defaultPropsValues[s];
                }
              }
            }
          }
          ...
          if (tweenProperty !== "scroll") {
            if (tweenProperty === "opacity" && isIE8) {
              // handle IE8 opacity
              var currentOpacity = getCurrentStyle(this[element], "filter");
              startValues["opacity"] =
                typeof currentOpacity === "number"
                  ? currentOpacity
                  : defaultPropsValues["opacity"];
            } else {
              if (coreProps[indexOf](tweenProperty) !== -1) {
                startValues[tweenProperty] =
                  getCurrentStyle(this[element], tweenProperty) ||
                  d[tweenProperty];
              } else {
                // plugins register here
                startValues[tweenProperty] =
                  tweenProperty in prepareStart
                    ? prepareStart[tweenProperty].call(
                        this,
                        tweenProperty,
                        this[valuesStart][tweenProperty]
                      )
                    : 0;
              }
            }
            ...
            startValues[tweenProperty] =
              this[element] === scrollContainer
                ? g.pageYOffset || scrollContainer.scrollTop
                : this[element].scrollTop;
          }
        }
      }
      for (var currentProperty in currentStyle) {
        // also add to startValues values from previous tweens
        if (
          transformFunctions[indexOf](currentProperty) !== -1 &&
          !(currentProperty in this[valuesStart])
        ) {
          startValues[currentProperty] =
            currentStyle[currentProperty] ||
            defaultPropsValues[currentProperty];
        }
      }
      ...
      if (transformProperty in this[valuesEnd]) {
        ...
        for (var sp in this[valuesStart][transformProperty]) {
          ...
          if (sp !== "perspective") {
            if (typeof this[valuesStart][transformProperty][sp] === "object") {
              for (var spp in this[valuesStart][transformProperty][sp]) {
                // 3rd level
                if (
                  typeof this[valuesEnd][transformProperty][sp] === "undefined"
                ) {
                  this[valuesEnd][transformProperty][sp] = {};
                }
                if (
                  typeof this[valuesStart][transformProperty][sp][spp] ===
                    "number" &&
                  typeof this[valuesEnd][transformProperty][sp][spp] ===
                    "undefined"
                ) {
                  this[valuesEnd][transformProperty][sp][spp] = this[
                    valuesStart
                  ][transformProperty][sp][spp];
                }
              }
            } else if (
              typeof this[valuesStart][transformProperty][sp] === "number"
            ) {
              if (
                typeof this[valuesEnd][transformProperty][sp] === "undefined"
              ) {
                // scale
                this[valuesEnd][transformProperty][sp] = this[valuesStart][
                  transformProperty
                ][sp];
              }
            }
  ...
  easingFn.linear = function(t) {
    return t;
  };
  easingFn.easingSinusoidalIn = function(t) {
    return -Math.cos((t * Math.PI) / 2) + 1;
  };
  easingFn.easingSinusoidalOut = function(t) {
    return Math.sin((t * Math.PI) / 2);
  };
  easingFn.easingSinusoidalInOut = function(t) {
    return -0.5 * (Math.cos(Math.PI * t) - 1);
  };
  easingFn.easingQuadraticIn = function(t) {
    return t * t;
  };
  easingFn.easingQuadraticOut = function(t) {
    return t * (2 - t);
  };
  easingFn.easingQuadraticInOut = function(t) {
    return t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t;
  };
  easingFn.easingCubicIn = function(t) {
    return t * t * t;
  };
  easingFn.easingCubicOut = function(t) {
    return --t * t * t + 1;
  };
  easingFn.easingCubicInOut = function(t) {
    return t < 0.5 ? 4 * t * t * t : (t - 1) * (2 * t - 2) * (2 * t - 2) + 1;
  };
  easingFn.easingQuarticIn = function(t) {
    return t * t * t * t;
  };
  easingFn.easingQuarticOut = function(t) {
    return 1 - --t * t * t * t;
  };
  easingFn.easingQuarticInOut = function(t) {
    return t < 0.5 ? 8 * t * t * t * t : 1 - 8 * --t * t * t * t;
  };
  easingFn.easingQuinticIn = function(t) {
    return t * t * t * t * t;
  };
  easingFn.easingQuinticOut = function(t) {
    return 1 + --t * t * t * t * t;
  };
  easingFn.easingQuinticInOut = function(t) {
    return t < 0.5 ? 16 * t * t * t * t * t : 1 + 16 * --t * t * t * t * t;
  };
  easingFn.easingCircularIn = function(t) {
    return -(Math.sqrt(1 - t * t) - 1);
  };
  easingFn.easingCircularOut = function(t) {
    return Math.sqrt(1 - (t = t - 1) * t);
  };
  easingFn.easingCircularInOut = function(t) {
    return (t *= 2) < 1
      ? -0.5 * (Math.sqrt(1 - t * t) - 1)
      : 0.5 * (Math.sqrt(1 - (t -= 2) * t) + 1);
  };
  easingFn.easingExponentialIn = function(t) {
    return Math.pow(2, 10 * (t - 1)) - 0.001;
  };
  easingFn.easingExponentialOut = function(t) {
    return 1 - Math.pow(2, -10 * t);
  };
  easingFn.easingExponentialInOut = function(t) {
    return (t *= 2) < 1
      ? 0.5 * Math.pow(2, 10 * (t - 1))
      : 0.5 * (2 - Math.pow(2, -10 * (t - 1)));
  };
  easingFn.easingBackIn = function(t) {
    var s = 1.70158;
    return t * t * ((s + 1) * t - s);
  };
  easingFn.easingBackOut = function(t) {
    var s = 1.70158;
    return --t * t * ((s + 1) * t + s) + 1;
  };
  easingFn.easingBackInOut = function(t) {
    var s = 1.70158 * 1.525;
  ...
  easingFn.easingElasticIn = function(t) {
    ...
    if (!_kea || _kea < 1) {
      _kea = 1;
      s = _kep / 4;
    } else s = ((_kep * Math.asin(1 / _kea)) / Math.PI) * 2;
    return -(
      _kea *
      Math.pow(2, 10 * (t -= 1)) *
      Math.sin(((t - s) * Math.PI * 2) / _kep)
    );
  };
  easingFn.easingElasticOut = function(t) {
    ...
    return (
      _kea * Math.pow(2, -10 * t) * Math.sin(((t - s) * Math.PI * 2) / _kep) + 1
    );
  };
  ...
  easingFn.easingBounceIn = function(t) {
    return 1 - easingFn.easingBounceOut(1 - t);
  };
  ...
  easingFn.easingBounceInOut = function(t) {
    if (t < 0.5) return easingFn.easingBounceIn(t * 2) * 0.5;
    return easingFn.easingBounceOut(t * 2 - 1) * 0.5 + 0.5;
  };
  ...
  var Tween = function(targetElement, startObject, endObject, optionsObj) {
      this[element] =
        "scroll" in endObject &&
        (targetElement === undefined || targetElement === null)
          ? scrollContainer
          : targetElement; // element animation is applied to
      ...
      if (
        this[options].perspective !== undefined &&
        transformProperty in this[valuesEnd]
      ) {
        // element transform perspective
        var perspectiveString =
          "perspective(" + parseInt(this[options].perspective) + "px)";
        this[valuesEnd][transformProperty].perspective = perspectiveString;
      }

      for (var repeatProp in this[valuesEnd]) {
        if (repeatProp in crossCheck && !this[options].rpr)
          crossCheck[repeatProp].call(this); // this is where we do the valuesStart and valuesEnd check for fromTo() method
      }

      this[options][chain] = []; // chained Tweens
      this[options][easing] =
        processEasing(optionsObj[easing]) ||
        easingFn[defaultOptions[easing]] ||
        easingFn["linear"]; // you can only set a core easing function as default
      this[options][repeat] = optionsObj[repeat] || defaultOptions[repeat];
      this[options][repeatDelay] =
        optionsObj[repeatDelay] || defaultOptions[repeatDelay];
      this[options][yoyo] = optionsObj[yoyo] || defaultOptions[yoyo];
      this[options][duration] =
        optionsObj[duration] || defaultOptions[duration]; // duration option | default
    ...
    TweenProto = (Tween.prototype = {
      ...
      start: function(t) {
        ...
        for (var endProp in this[valuesEnd]) {
          if (endProp in crossCheck && this[options].rpr)
            crossCheck[endProp].call(this); // this is where we do the valuesStart and valuesEnd check for to() method
    ...
    TweensTO = function(els, vE, o) {
      ...
      for (var i = 0, tl = els[length]; i < tl; i++) {
        optionsObj[i] = o || {};
        o[delay] = o[delay] || defaultOptions[delay];
        optionsObj[i][delay] =
          i > 0 ? o[delay] + (o[offset] || defaultOptions[offset]) : o[delay];
    ...
    ws = (TweensTO.prototype = TweensFT.prototype = {
      ...
      chain: function() {
        this.tweens[this.tweens[length] - 1][options][chain] = arguments;
        return this;
      },
      play: function() {
        for (var i = 0, tl = this.tweens[length]; i < tl; i++) {
  ...
  return {
    // export core methods to public for plugins
    property: property,
    getPrefix: getPrefix,
    selector: selector,
