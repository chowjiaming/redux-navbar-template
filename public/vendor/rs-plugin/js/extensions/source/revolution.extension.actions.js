...
(function($) {
      ...
      jQuery.each(as, function(i, a) {
        ...
        switch (a.action) {
          ...
          case "toggle_global_mute_video":
            ...
            break;
          case "toggleslider":
            if (opt.slidertoggledby == undefined)
              opt.slidertoggledby = new Array();
            opt.slidertoggledby.push(_nc);
            break;
          case "togglefullscreen":
            if (opt.fullscreentoggledby == undefined)
              opt.fullscreentoggledby = new Array();
            opt.fullscreentoggledby.push(_nc);
            break;
        }

        _nc.on(a.event, function() {
          if (a.event === "click" && _nc.hasClass("tp-temporarydisabled"))
            return false;
          var tnc =
            a.layer == "backgroundvideo"
              ? jQuery(
          ...
          if (
            ...
            if (tnc.length > 0) {
              ...
              if (
                a.action == "startlayer" ||
                (a.action == "togglelayer" && tnc.data("animdirection") != "in")
              ) {
                _.animdirection = "in";
                _.triggerstate = "on";
            ...
            if (
              ...
              (a.action == "playvideo" ||
                a.action == "stopvideo" ||
                a.action == "togglevideo" ||
                a.action == "mutevideo" ||
                a.action == "unmutevideo" ||
                a.action == "toggle_mute_video" ||
                a.action == "toggle_global_mute_video")
            ) {
        ...
        switch (a.action) {
          ...
          case "stoplayer":
            ...
            if (
              tnc.length > 0 &&
              d !== undefined &&
              ((d.frames !== undefined && d.frames[0].delay != "bytrigger") ||
                (d.frames === undefined && d.start !== "bytrigger"))
            ) {
              d.triggerstate = "on";
              //d.animdirection="in";
            }
  ...
  var actionSwitches = function(tnc, opt, a, _nc) {
    switch (a.action) {
      ...
      case "togglevideo":
        if (tnc.length > 0)
          if (!_R.isVideoPlaying(tnc, opt)) _R.playVideo(tnc, opt);
      ...
      case "toggleclass":
        if (tnc.length > 0)
          if (!tnc.hasClass(a.classname)) tnc.addClass(a.classname);
          else tnc.removeClass(a.classname);
        break;
      case "gofullscreen":
      case "exitfullscreen":
      case "togglefullscreen":
        if (
          ...
          (a.action == "togglefullscreen" || a.action == "exitfullscreen")
        ) {
          ...
          jQuery(".rs-go-fullscreen").length == 0 &&
          (a.action == "togglefullscreen" || a.action == "gofullscreen")
        ) {
          var gf =
            opt.c.closest(".forcefullwidth_wrapper_tp_banner").length > 0
              ? opt.c.closest(".forcefullwidth_wrapper_tp_banner")
