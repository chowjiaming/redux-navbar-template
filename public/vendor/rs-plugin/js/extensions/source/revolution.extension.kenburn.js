...
(function($) {
  ...
  jQuery.extend(true, _R, {
    ...
    startKenBurn: function(l, opt, prgs) {
      ...
      var getKBSides = function(w, h, f, cw, ch, ho, vo) {
        ...
        kcalcL = function(cw, ch, d) {
          var f = d.scalestart / 100,
            fe = d.scaleend / 100,
            ofs =
              d.offsetstart != undefined
                ? d.offsetstart.split(" ") || [0, 0]
                : [0, 0],
            ofe =
              d.offsetend != undefined
                ? d.offsetend.split(" ") || [0, 0]
                : [0, 0];
          d.bgposition =
            d.bgposition == "center center" ? "50% 50%" : d.bgposition;
      ...
      kbtl.add(
        punchgs.TweenLite.fromTo(k, d.duration / 1000, anim.start, anim.end),
        0
      );
      kbtl.add(
        punchgs.TweenLite.fromTo(kw, d.duration / 1000, anim.starto, anim.endo),
        0
      );

      // ADD BLUR EFFECT ON THE ELEMENTS
      if (
        d.blurstart !== undefined &&
        d.blurend !== undefined &&
        (d.blurstart !== 0 || d.blurend !== 0)
      ) {
        var blurElement = { a: d.blurstart },
          blurElementEnd = { a: d.blurend, ease: anim.endo.ease },
          blurAnimation = new punchgs.TweenLite(
            blurElement,
            d.duration / 1000,
            blurElementEnd
          );

        blurAnimation.eventCallback(
          "onUpdate",
          function(kw) {
