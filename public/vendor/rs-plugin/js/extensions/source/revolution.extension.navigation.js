...
(function($) {
  ...
  jQuery.extend(true, _R, {
    hideUnHideNav: function(opt) {
      ...
      if (ckNO(a))
        biggerNav(opt.c.find(".tparrows"), a.hide_under, w, a.hide_over);
      if (ckNO(b))
        biggerNav(opt.c.find(".tp-bullets"), b.hide_under, w, b.hide_over);
      if (ckNO(c))
        biggerNav(
          opt.c.parent().find(".tp-thumbs"),
          c.hide_under,
          w,
          c.hide_over
        );
    ...
    resizeThumbsTabs: function(opt, force) {
      if (
        (opt.navigation && opt.navigation.tabs.enable) ||
        ...
        if (ckNO(otab) && (force || otab.width > otab.min_width))
          rtt(f, tws, opt.c, otab, opt.slideamount, "tab");
        if (ckNO(othu) && (force || othu.width > othu.min_width))
          rtt(f, tws, opt.c, othu, opt.slideamount, "thumb");
        if (ckNO(otbu) && force) {
          ...
          bw.find(".tp-bullet").each(function(i) {
            var b = jQuery(this),
              ...
              w =
                b.outerWidth() +
                parseInt(otbu.space === undefined ? 0 : otbu.space, 0),
              h =
                b.outerHeight() +
                parseInt(otbu.space === undefined ? 0 : otbu.space, 0);
            ...
            if (otbu.direction === "vertical") {
              ...
              bw.css({
                height: (am - 1) * h + b.outerHeight(),
                width: b.outerWidth()
              });
            } else {
              b.css({ left: (am - 1) * w + "px", top: "0px" });
              bw.css({
                width: (am - 1) * w + b.outerWidth(),
      ...
      if (ckNO(opt.navigation.bullets)) {
        if (
          ...
          opt.navigation.bullets.h_offset_old =
            opt.navigation.bullets.h_offset_old === undefined
              ? opt.navigation.bullets.h_offset
              : opt.navigation.bullets.h_offset_old;
          opt.navigation.bullets.h_offset =
            opt.navigation.bullets.h_align === "center"
              ? opt.navigation.bullets.h_offset_old + lof / 2 - rof / 2
              : opt.navigation.bullets.h_offset_old + lof - rof;
        }
        setNavElPositions(
          opt.c.find(".tp-bullets"),
          opt.navigation.bullets,
          opt
        );
      }
      ...
      if (ckNO(opt.navigation.arrows)) {
        if (
          ...
          opt.navigation.arrows.left.h_offset_old =
            opt.navigation.arrows.left.h_offset_old === undefined
              ? opt.navigation.arrows.left.h_offset
              : opt.navigation.arrows.left.h_offset_old;
          opt.navigation.arrows.left.h_offset =
            opt.navigation.arrows.left.h_align === "right"
          ...
          opt.navigation.arrows.right.h_offset_old =
            opt.navigation.arrows.right.h_offset_old === undefined
              ? opt.navigation.arrows.right.h_offset
              : opt.navigation.arrows.right.h_offset_old;
          opt.navigation.arrows.right.h_offset =
            opt.navigation.arrows.right.h_align === "right"
              ? opt.navigation.arrows.right.h_offset_old + rof
              : opt.navigation.arrows.right.h_offset_old + lof;
        }
        setNavElPositions(
          opt.c.find(".tp-leftarrow.tparrows"),
          opt.navigation.arrows.left,
          opt
        );
        setNavElPositions(
          opt.c.find(".tp-rightarrow.tparrows"),
          opt.navigation.arrows.right,
          opt
        );
      }

      if (ckNO(opt.navigation.thumbnails))
        moveThumbsInPosition(
          opt.c.parent().find(".tp-thumbs"),
          opt.navigation.thumbnails
        );
    ...
    createNavigation: function(container, opt) {
      ...
      opt.li.each(function(index) {
        ...
        if (b)
          if (opt.navigation.bullets.rtl) addBullet(container, _b, li_rtl, opt);
          else addBullet(container, _b, li, opt);

        if (c)
          if (opt.navigation.thumbnails.rtl)
            addThumb(container, _c, li_rtl, "tp-thumb", opt);
          else addThumb(container, _c, li, "tp-thumb", opt);
        if (d)
          if (opt.navigation.tabs.rtl)
            addThumb(container, _d, li_rtl, "tp-tab", opt);
          else addThumb(container, _d, li, "tp-tab", opt);
      });
      ...
      container.bind(
        ...
        function() {
          ...
          container.find(".tp-bullet").each(function() {
            var _t = jQuery(this);
            if (_t.data("liref") === si) _t.addClass("selected");
          ...
          cp.find(".tp-thumb, .tp-tab").each(function() {
            ...
            if (_t.data("liref") === si) {
              ...
              else moveThumbsInPosition(cp.find(".tp-thumbs"), _c);
            } else _t.removeClass("selected");
          });
          ...
          if (_a.enable === true) {
            ...
            if (!_a.rtl) {
              punchgs.TweenLite.set(_a.left.j.find(".tp-arr-imgholder"), {
                backgroundImage: "url(" + opt.thumbs[pi].src + ")"
              });
              punchgs.TweenLite.set(_a.right.j.find(".tp-arr-imgholder"), {
                backgroundImage: "url(" + opt.thumbs[ni].src + ")"
              });
            } else {
              punchgs.TweenLite.set(_a.left.j.find(".tp-arr-imgholder"), {
                backgroundImage: "url(" + opt.thumbs[ni].src + ")"
      ...
      cp.on("mouseenter mousemove", function() {
        if (!cp.hasClass("tp-mouseover")) {
          ...
          if (a && _a.hide_onleave)
            showHideNavElements(cp.find(".tparrows"), _a, "show");
          if (b && _b.hide_onleave)
            showHideNavElements(cp.find(".tp-bullets"), _b, "show");
          if (c && _c.hide_onleave)
            showHideNavElements(cp.find(".tp-thumbs"), _c, "show");
          if (d && _d.hide_onleave)
            showHideNavElements(cp.find(".tp-tabs"), _d, "show");
      ...
      if (a && _a.hide_onleave)
        showHideNavElements(cp.find(".tparrows"), _a, "hide", 0);
      if (b && _b.hide_onleave)
        showHideNavElements(cp.find(".tp-bullets"), _b, "hide", 0);
      if (c && _c.hide_onleave)
        showHideNavElements(cp.find(".tp-thumbs"), _c, "hide", 0);
      if (d && _d.hide_onleave)
        showHideNavElements(cp.find(".tp-tabs"), _d, "hide", 0);
      ...
      if (
        opt.navigation.touch.touchOnDesktop === "on" ||
        (opt.navigation.touch.touchenabled == "on" && _ISM)
      )
        swipeAction(container, opt, "swipebased");
    }
  });
  ...
  var moveThumbsInPosition = function(container, opt) {
    var thumbs = container.hasClass("tp-thumbs") ? ".tp-thumbs" : ".tp-tabs",
      thumbmask = container.hasClass("tp-thumbs")
        ? ".tp-thumb-mask"
        : ".tp-tab-mask",
      thumbsiw = container.hasClass("tp-thumbs")
      ...
      tw =
        thumbdir === "vertical"
          ? t
              .find(thumb)
              .first()
              .outerHeight(true) + opt.space
          : t
              .find(thumb)
              .first()
              .outerWidth(true) + opt.space,
    ...
    if (me > 2) {
      curpos =
        tp - (elp + tw) <= 0
          ...
          : curpos;
      curpos =
        tp - tw + elp + tmw < tw && tp + (Math.round(me) - 2) * tw < elp
          ? tp + (Math.round(me) - 2) * tw
          : curpos;
    }
  ...
  var rtt = function(f, tws, c, o, lis, wh) {
    ...
    tws.add(
      punchgs.TweenLite.set(ins, { width: iw + "px", height: ih + "px" })
    );
    tws.add(
      punchgs.TweenLite.set(mask, { width: iw + "px", height: ih + "px" })
    );
  ...
  var initKeyboard = function(container, opt) {
    ...
    jQuery(document).keydown(function(e) {
      if (
        (opt.navigation.keyboard_direction == "horizontal" &&
          e.keyCode == 39) ||
  ...
  var initMouseScroll = function(container, opt) {
    if (
      opt.navigation.mouseScrollNavigation !== "on" &&
      opt.navigation.mouseScrollNavigation !== "carousel"
    )
      return;
    ...
    container.on("mousewheel DOMMouseScroll", function(e) {
      var res = normalizeWheel(e.originalEvent),
        ...
        ls =
          (asi != -1 && asi == opt.slideamount - 1) ||
          (psi != 1 && psi == opt.slideamount - 1)
            ? true
            : false,
        ret = true;
      ...
      if (opt.navigation.mouseScrollNavigation != "carousel") {
        if (opt.navigation.mouseScrollReverse !== "reverse")
          if (
            (tc > 0 && res.pixelY > 0) ||
            (bc < jQuery(window).height() && res.pixelY < 0)
          )
            ret = true;
        if (opt.navigation.mouseScrollReverse === "reverse")
          if (
  ...
  var isme = function(a, c, e) {
    a = _ISM
      ? jQuery(e.target).closest("." + a).length ||
        jQuery(e.srcElement).closest("." + a).length
      : jQuery(e.toElement).closest("." + a).length ||
        jQuery(e.originalTarget).closest("." + a).length;
  ...
  var swipeAction = function(container, opt, vertical) {
    ...
    var notonbody = _ISM || _R.get_browser() === "Firefox",
      ...
      pagescroll =
        opt.navigation.thumbnails.direction === "vertical" ||
        opt.navigation.tabs.direction === "vertical"
          ? "none"
          : "vertical",
      swipe_wait_dir = opt.navigation.touch.swipe_direction || "horizontal";

    pagescroll =
      vertical == "swipebased" && swipe_wait_dir == "vertical"
        ? "none"
        : vertical
        ? "vertical"
        : pagescroll;
      ...
      swipeStatus: function(
        ...
        if (
          ...
          (((phase === "move" || phase === "end" || phase == "cancel") &&
            (opt.dragStartedOverSlider &&
              !opt.dragStartedOverThumbs &&
              !opt.dragStartedOverTabs)) ||
            (phase === "start" &&
              withinslider > 0 &&
              withinthumbs === 0 &&
              withintabs === 0))
          ...
          distance =
            direction && direction.match(/left|up/g)
          ...
          switch (phase) {
            case "start":
              if (_.positionanim !== undefined) {
                _.positionanim.kill();
                _.slide_globaloffset =
                  _.infinity === "off"
                    ? _.slide_offset
                    : _R.simp(_.slide_offset, _.maxwidth);
              }
              _.overpull = "none";
            ...
            case "move":
              ...
              _.slide_offset =
                _.infinity === "off"
                  ? _.slide_globaloffset + distance
                  : _R.simp(_.slide_globaloffset + distance, _.maxwidth);

              if (_.infinity === "off") {
                var bb =
                  _.horizontal_align === "center"
                    ? (_.wrapwidth / 2 - _.slide_width / 2 - _.slide_offset) /
                      _.slide_width
                    : (0 - _.slide_offset) / _.slide_width;

                if (
                  (_.overpull === "none" || _.overpull === 0) &&
                  (bb < 0 || bb > opt.slideamount - 1)
                )
                  _.overpull = distance;
                else if (
                  bb >= 0 &&
                  bb <= opt.slideamount - 1 &&
                  ((bb >= 0 && distance > _.overpull) ||
                    (bb <= opt.slideamount - 1 && distance < _.overpull))
                )
                  _.overpull = 0;
                ...
                _.slide_offset =
                  bb < 0
                    ? _.slide_offset +
                      (_.overpull - distance) / 1.1 +
                      Math.sqrt(Math.abs((_.overpull - distance) / 1.1))
                    : bb > opt.slideamount - 1
                    ? _.slide_offset +
                      (_.overpull - distance) / 1.1 -
                      Math.sqrt(Math.abs((_.overpull - distance) / 1.1))
        ...
        // SWIPE OVER THUMBS OR TABS
          ...
          ((phase === "move" || phase === "end" || phase == "cancel") &&
            (!opt.dragStartedOverSlider &&
              (opt.dragStartedOverThumbs || opt.dragStartedOverTabs))) ||
          (phase === "start" &&
            (withinslider > 0 && (withinthumbs > 0 || withintabs > 0)))
        ) {
          ...
          var thumbs = opt.dragStartedOverThumbs ? ".tp-thumbs" : ".tp-tabs",
            thumbmask = opt.dragStartedOverThumbs
              ? ".tp-thumb-mask"
              : ".tp-tab-mask",
            thumbsiw = opt.dragStartedOverThumbs
              ? ".tp-thumbs-inner-wrapper"
              : ".tp-tabs-inner-wrapper",
            thumb = opt.dragStartedOverThumbs ? ".tp-thumb" : ".tp-tab",
            _o = opt.dragStartedOverThumbs
              ? opt.navigation.thumbnails
              : opt.navigation.tabs;
            ...
            tw =
              tdir === "vertical"
                ? t
                    .find(thumb)
                    .first()
                    .outerHeight(true) + _o.space
                : t
                    .find(thumb)
                    .first()
                    .outerWidth(true) + _o.space,
            newpos =
              el.data("offset") === undefined
                ? 0
                : parseInt(el.data("offset"), 0),
            curpos = 0;
          ...
          switch (phase) {
            case "start":
              container
                ...
                .addClass("dragged");
              newpos =
                tdir === "vertical" ? el.position().top : el.position().left;
              ...
              curpos =
                curpos > 0
                  ? tdir === "horizontal"
                    ? curpos -
                      el.width() *
                        (((curpos / el.width()) * curpos) / el.width())
                    : curpos -
                      el.height() *
                        (((curpos / el.height()) * curpos) / el.height())
                  : curpos;
              var dif =
                tdir === "vertical"
                  ? 0 - (el.height() - t.height())
                  : 0 - (el.width() - t.width());
              curpos =
                curpos < dif
                  ? tdir === "horizontal"
                    ? curpos +
                      (((el.width() * (curpos - dif)) / el.width()) *
                        (curpos - dif)) /
                        el.width()
                    : curpos +
                      (((el.height() * (curpos - dif)) / el.height()) *
                        (curpos - dif)) /
                        el.height()
                  : curpos;
              if (tdir === "vertical")
                punchgs.TweenLite.set(el, { top: curpos + "px" });
              else punchgs.TweenLite.set(el, { left: curpos + "px" });
            ...
            case "cancel":
              if (istt) {
                ...
                curpos =
                  tdir === "vertical"
                    ? curpos < 0 - (el.height() - t.height())
                      ? 0 - (el.height() - t.height())
                      : curpos
                    : curpos < 0 - (el.width() - t.width())
                    ? 0 - (el.width() - t.width())
                    : curpos;
                curpos = curpos > 0 ? 0 : curpos;

                curpos =
                  Math.abs(distance) > tw / 10
                    ? distance <= 0
                      ? Math.floor(curpos / tw) * tw
                      : Math.ceil(curpos / tw) * tw
                    : distance < 0
                    ? Math.ceil(curpos / tw) * tw
                    : Math.floor(curpos / tw) * tw;
          ...
          if (phase == "end" && !istt) {
            ...
            if (
              (swipe_wait_dir == "horizontal" && direction == "left") ||
              (swipe_wait_dir == "vertical" && direction == "up")
            ) {
              opt.sc_indicator_dir = 0;
            ...
            if (
              (swipe_wait_dir == "horizontal" && direction == "right") ||
              (swipe_wait_dir == "vertical" && direction == "down")
            ) {
              opt.sc_indicator_dir = 1;
              _R.callingNewSlide(opt.c, -1);
              return false;
  ...
  var hdResets = function(o) {
    o.hide_delay = !jQuery.isNumeric(parseInt(o.hide_delay, 0))
      ? 0.2
      : o.hide_delay / 1000;
    o.hide_delay_mobile = !jQuery.isNumeric(parseInt(o.hide_delay_mobile, 0))
      ? 0.2
      : o.hide_delay_mobile / 1000;
  };
  ...
  var ckNOLO = function(opt) {
    return (
      opt &&
      opt.enable &&
      opt.hide_onleave === true &&
      (opt.position === undefined ? true : !opt.position.match(/outer/g))
    );
  };
  ...
  var callAllDelayedCalls = function(container, opt) {
    ...
    if (ckNOLO(opt.navigation.arrows))
      punchgs.TweenLite.delayedCall(
        _ISM
          ? opt.navigation.arrows.hide_delay_mobile
          : opt.navigation.arrows.hide_delay,
        showHideNavElements,
        [cp.find(".tparrows"), opt.navigation.arrows, "hide"]
      );

    if (ckNOLO(opt.navigation.bullets))
      punchgs.TweenLite.delayedCall(
        _ISM
          ? opt.navigation.bullets.hide_delay_mobile
          : opt.navigation.bullets.hide_delay,
        showHideNavElements,
        [cp.find(".tp-bullets"), opt.navigation.bullets, "hide"]
      );

    if (ckNOLO(opt.navigation.thumbnails))
      punchgs.TweenLite.delayedCall(
        _ISM
          ? opt.navigation.thumbnails.hide_delay_mobile
          : opt.navigation.thumbnails.hide_delay,
        showHideNavElements,
        [cp.find(".tp-thumbs"), opt.navigation.thumbnails, "hide"]
      );

    if (ckNOLO(opt.navigation.tabs))
      punchgs.TweenLite.delayedCall(
        _ISM
          ? opt.navigation.tabs.hide_delay_mobile
          : opt.navigation.tabs.hide_delay,
        showHideNavElements,
        [cp.find(".tp-tabs"), opt.navigation.tabs, "hide"]
      );
  };
  ...
  var putVinPosition = function(el, o, opt) {
    var elh = el.outerHeight(true),
      ...
      by =
        o.container == "layergrid"
          ? opt.sliderLayout == "fullscreen"
            ? opt.height / 2 - (opt.gridheight[opt.curWinRange] * opt.bh) / 2
            : opt.autoHeight == "on" ||
              (opt.minHeight != undefined && opt.minHeight > 0)
            ? oh / 2 - (opt.gridheight[opt.curWinRange] * opt.bh) / 2
            : 0
          : 0,
      a =
        o.v_align === "top"
  ...
  var setNavElPositions = function(el, o, opt) {
    ...
    if (
      o.position === "outer-left" &&
      (o.sliderLayout == "fullwidth" || o.sliderLayout == "fullscreen")
    )
      punchgs.TweenLite.set(el, {
        left: 0 - el.outerWidth() + "px",
        x: o.h_offset + "px"
      });
    else if (
      o.position === "outer-right" &&
      (o.sliderLayout == "fullwidth" || o.sliderLayout == "fullscreen")
    )
      punchgs.TweenLite.set(el, {
        right: 0 - el.outerWidth() + "px",
        x: o.h_offset + "px"
      });
    ...
    if (el.hasClass("tp-thumbs") || el.hasClass("tp-tabs")) {
      var wpad = el.data("wr_padding"),
        maxw = el.data("maxw"),
        maxh = el.data("maxh"),
        mask = el.hasClass("tp-thumbs")
          ? el.find(".tp-thumb-mask")
          : el.find(".tp-tab-mask"),
      ...
      if (
        maxh + 2 * wpad > wh &&
        o.position !== "outer-bottom" &&
        o.position !== "outer-top"
      ) {
        punchgs.TweenLite.set(el, {
          top: "0px",
          y: 0,
  ...
  var addBullet = function(container, o, li, opt) {
    ...
    if (container.find(".tp-bullets").length === 0) {
      o.style = o.style === undefined ? "" : o.style;
      container.append(
        '<div class="tp-bullets ' + o.style + " " + o.direction + '"></div>'
      );
    }
    ...
    b.find(".tp-bullet-image").css({
      backgroundImage: "url(" + opt.thumbs[li.index()].src + ")"
    });
  ...
  var addThumb = function(container, o, li, what, opt) {
    ...
    o.visibleAmount =
      o.visibleAmount > opt.slideamount ? opt.slideamount : o.visibleAmount;
    ...
    if (container.parent().find(thumbs).length === 0) {
      ...
      if (o.position === "outer-top") container.parent().prepend(addcontent);
      else if (o.position === "outer-bottom") container.after(addcontent);
      else container.append(addcontent);
    ...
    var linkto = li.data("index"),
      ...
      maxw =
        o.direction === "horizontal"
          ? o.width * o.visibleAmount + o.space * (o.visibleAmount - 1)
          : o.width,
      maxh =
        o.direction === "horizontal"
          ? o.height
          : o.height * o.visibleAmount + o.space * (o.visibleAmount - 1),
      inst = o.tmp;
    jQuery.each(opt.thumbs[li.index()].params, function(i, obj) {
      inst = inst.replace(obj.from, obj.to);
    });
    ...
    var position =
        o.position === "outer-top" || o.position === "outer-bottom"
          ? "relative"
          : "absolute",
      _margin =
        (o.position === "outer-top" || o.position === "outer-bottom") &&
    ...
    t.css({
      maxWidth: maxw + "px",
      /*margin:_margin, */ maxHeight: maxh + "px",
      overflow: "visible",
      position: position,
  ...
  var setONHeights = function(o) {
    ...
    o.top_outer = !ot.hasClass("tp-forcenotvisible")
      ? ot.outerHeight() || 0
      : 0;
    o.bottom_outer = !ob.hasClass("tp-forcenotvisible")
      ? ob.outerHeight() || 0
      : 0;
  };
  ...
  var biggerNav = function(el, a, b, c) {
    if (a > b || b > c) el.addClass("tp-forcenotvisible");
    else el.removeClass("tp-forcenotvisible");
  };
})(jQuery);
