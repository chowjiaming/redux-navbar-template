...
(function() {
  ...
  window.RsLiquideffectAddOn = function(_$, slider) {
    ...
    var effects = slider.find("li[data-liquideffect]").each(function() {
      ...
      if (!/all|smart/.test(slider[0].opt.lazyType))
        slider[0].opt.lazyType = "smart";
      if (!img.attr("data-lazyload")) {
        sprite = img.attr("src");
        img.attr("data-lazyload", sprite);
        img.attr(
          "src",
          base.replace("/extensions", "") +
            "/revslider/admin/assets/images/dummy.png"
        );
      } else {
  ...
  function LiquidEffect(slider, effects) {
    ...
    slider
      .one("revolution.slide.onloaded", this.onLoaded.bind(this))
  ...
  LiquidEffect.prototype = {
    onLoaded: function() {
      this.slider
        .on("revolution.slide.onbeforeswap", this.beforeChange.bind(this))
    ...
    onChange: function(e, data) {
      ...
      var canvas = data.prevslide
        .removeClass("liquid-force-visible")
        .data("liquideffectcanvas");
      if (canvas) {
        canvas.ticker.stop();
    ...
    beforeChange: function(e, data) {
      ...
      function loadImage($this, slide) {
        ...
        img.src = bgImg
          ...
          .replace(/"/g, "");
        img.onload = function() {
          var canvas = new LiquidCanvas(
            slide,
            parseInt(img.naturalWidth, 10),
            parseInt(img.naturalHeight, 10)
          );
      ...
      if (this.effects)
        this.effects.each(function() {
          jQuery(this).removeData(
            "liquideffectcanvas liquideffectsettings liquideffectorig"
          );
        });
  ...
  LiquidCanvas.prototype = {
    tick: function(delta) {
      if (this.settings.autoplay) {
        if (this.settings.speedx)
          this.displacement.x += this.settings.speedx * delta;
        if (this.settings.speedy) this.displacement.y += this.settings.speedy;
        if (this.settings.rotationx)
          this.displacement.rotation.x += this.settings.rotationx;
        if (this.settings.rotationy)
          this.displacement.rotation.y += this.settings.rotationy;
        if (this.settings.rotation)
          this.displacement.rotation +=
            (this.settings.rotation * Math.PI) / 180;
      }
    ...
    onClick: function() {
      ...
      if (this.settings.interscalex || this.settings.interscaley) {
        ...
        if (this.settings.interscalex)
          obj.x = this.orig.scalex + this.settings.interscalex;
        if (this.settings.interscaley)
          obj.y = this.orig.scaley + this.settings.interscaley;
    ...
    transitionIn: function() {
      ...
      if (this.interactive && this.event === "mousedown")
        obj1.onComplete = this.eventsReady.bind(this);
    ...
    animateIn: function(first) {
      ...
      this.started = true;
      this.del =
        this.settings.transcross || first
          ? this.settings.transtime * 0.001 * 0.5
          : 0;
