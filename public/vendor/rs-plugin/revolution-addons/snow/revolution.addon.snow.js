...
var RsSnowAddOn = function($, slider) {
  ...
  options = {
    ...
    particleOpacity: [
      parseFloat(options.minOpacity),
      parseFloat(options.maxOpacity)
    ],
    particleSpeed: [
      parseInt(options.minSpeed, 10),
      parseInt(options.maxSpeed, 10)
    ],
    particleSinus: [
      parseInt(options.minSinus, 10),
      parseInt(options.maxSinus, 10)
    ]
  };
...
(function(e, s) {
  ...
  function r(e) {
    ...
    for (var r in e.snowflakes)
      if (
        (e.snowflakes[r].y + 0.1 * e.snowflakes[r].r < 0 && 1 == e.summer) ||
        (e.snowflakes[r].y > e.h + e.snowflakes[r].r && 1 == e.summer)
      ) {
      } else {
        (e.snowflakes[r].delta +=
          ...
          e.summer
            ? (e.snowflakes[r].y +=
                e.snowflakes[r].speed / 50 + 0.1 * e.snowflakes[r].r)
            : (e.snowflakes[r].y +=
                e.snowflakes[r].speed / 100 + 0.1 * e.snowflakes[r].r),
          (e.snowflakes[r].x +=
            Math.sin(e.snowflakes[r].delta) * (0.1 * e.snowflakes[r].r)),
        ...
        (o = 0.1 > o ? 0.1 : o),
          ...
          (e.snowflakes[r].x =
            e.snowflakes[r].x > e.w + e.snowflakes[r].r
              ...
              : e.snowflakes[r].x),
          e.ctx.beginPath(),
          e.ctx.arc(e.snowflakes[r].x, e.snowflakes[r].y, o, 2 * Math.PI, !1),
  ...
  function i(e) {
    ...
    return (
      (s.delta =
        (e.particleSinus[0] +
          Math.random() * (e.particleSinus[1] - e.particleSinus[0])) *
        Math.round(2 * Math.random() - 1)),
      (s.r =
        e.particlaSize[0] +
        Math.random() * (e.particlaSize[1] - e.particlaSize[0])),
      (s.alpha =
        e.particleOpacity[0] +
        Math.random() * (e.particleOpacity[1] - e.particleOpacity[0])),
      (s.speed =
        ((e.particleSpeed[0] +
          Math.random() * (e.particleSpeed[1] - e.particleSpeed[0])) *
          s.r) /
        3),
      (s.x = Math.random() * e.w),
      (s.y = Math.random() * -e.h),
  ...
  e.fn.extend({
    letItSnow: function(r) {
      ...
      return (
        ...
        this.each(function() {
          if (
            -1 != e.inArray(r, ["destroy", "stop", "play", "winter", "summer"])
          ) {
            switch (r) {
              case "destroy":
          ...
          return (
            (r.c = e(this)),
            (r.sc = r.selector != s ? e(this).find(r.selector) : r.c),
            0 == r.sc.length
              ? !1
              ...
              : (r.sc.find(".snowflakes_wrapper").remove(),
                ...
                r.dimension != self ? (r.sizer = r.c) : (r.sizer = r.sc),
                (r.w = r.sizer.width()),
                (r.h = r.sizer.height()),
                r.sc.find(".snowflakes_wrapper_inner").css({
                  width: r.w,
                  height: r.h
                }),
                (r.canvas = r.canvas[0]),
