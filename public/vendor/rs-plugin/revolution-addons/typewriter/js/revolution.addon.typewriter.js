...
(function() {
  var $,
    ...
    anime =
      'content: "[char]"; animation: tp-typewriter [speed]s steps(2, start) infinite';
  ...
  function RsTypewriter(_$, slider, layrs) {
    ...
    if (levels && levels.hasOwnProperty("responsiveLevels"))
      levels = levels.responsiveLevels;
    ...
    writeLayerData: function(i, el) {
      ...
      if (el.getAttribute("data-whiteboard")) {
        if (typeof console !== "undefined") {
          console.log(
            "Whiteboard is disabled for Layers with Typewriter Add-On effect enabled"
          );
        }
      ...
      this.layrs[this.layrs.length] = new TypewriterLayer(
        el,
        txt,
        this.levels,
        this.slider
  ...
  function TypewriterLayer(el, txt, levels, revapi) {
    ...
    if (this.blinking) {
      ...
      css +=
        ...
        ":after {" +
        anime
          .replace("[char]", this.cursorType === "one" ? "_" : "|")
          ...
          for (i = 0; i < len; i++) {
            ...
            if (i > 0) {
              ...
              css +=
                ...
                "#" +
                el.id +
                ".tp-caption[data-typewriter]:after {position: " +
                auto +
                "}}";
            } else if (auto) {
  ...
  TypewriterLayer.prototype = {
    ...
    animate: function() {
      ...
      if (next) {
        ...
        this.running = false;
        if (this.hideCursor)
          this.el.className = this.el.className + " hide-typewriter-cursor";
    ...
    onRedraw: function(restore) {
      if (!restore) {
        var pos = window
          .getComputedStyle(this.el, ":after")
    ...
    resume: function() {
      ...
      if (!this.rpaused) {
        this.paused = false;
        if (this.lastSpeed)
          this.timer = setTimeout(this.animate.bind(this), this.lastSpeed);
        else this.animate();
      } else {
