...
(function($) {
  ...
  var init = function(opt) {
    ...
    var _css_thehand = "position:absolute;z-index:1000;top:0px;left:0px;",
      ...
      _css_hand_image =
        "position:absolute; top:0px;left:0px;background-size:contain;background-repeat:no-repeat;background-position:center center;";
    //opacity:0.2;border:1px dotted rgba(255,0,0,0.2);

    if (o.movehand != undefined)
      o.movehand.markup =
        '<div class="wb-move-hand wb-thehand" style="' +
        _css_thehand +
        '"><div class="wb-hand-inner" style="' +
        _css_hand_inner +
        ";width:" +
        o.movehand.width +
        "px;height:" +
        o.movehand.height +
        'px"><div class="wb-hand-scale" ><div class="wb-hand-image" style="' +
        _css_hand_image +
        "width:" +
        o.movehand.width +
        "px;height:" +
        ...
        o.movehand.src +
        ');"></div></div></div>';
    if (o.writehand != undefined)
      o.writehand.markup =
        '<div class="wb-draw-hand wb-thehand" style="' +
        _css_thehand +
        '"><div class="wb-hand-inner" style="' +
        _css_hand_inner +
        '"><div class="wb-hand-scale" style="' +
        _css_hand_scale +
        '"><div class="wb-hand-image" style="' +
        _css_hand_image +
        "width:" +
        o.writehand.width +
        "px;height:" +
    ...
    opt.c.on("revolution.layeraction", function(event, obj) {
      ...
      if (obj_wb != undefined) {
        if (obj_wb.configured != true) {
          if (
            obj_wb.hand_function == "write" ||
            obj_wb.hand_function == "draw"
          ) {
            obj_wb.jitter_distance =
              obj_wb.jitter_distance != undefined
                ? parseInt(obj_wb.jitter_distance, 0) / 100
                : parseInt(o.writehand.jittering.distance, 0) / 100;
            obj_wb.jitter_distance_horizontal =
              obj_wb.jitter_distance_horizontal != undefined
                ? parseInt(obj_wb.jitter_distance_horizontal, 0) / 100
                : parseInt(o.writehand.jittering.distance_horizontal, 0) / 100;
            obj_wb.jitter_offset =
              obj_wb.jitter_offset != undefined
                ? parseInt(obj_wb.jitter_offset, 0) / 100
                : parseInt(o.writehand.jittering.offset, 0) / 100;
            obj_wb.jitter_offset_horizontal =
              obj_wb.jitter_offset_horizontal != undefined
                ? parseInt(obj_wb.jitter_offset_horizontal, 0) / 100
                : parseInt(o.writehand.jittering.offset_horizontal, 0) / 100;
        ...
        if (obj.eventtype == "enterstage") {
          ...
          if (!obj.layer.hasClass("handadded")) {
            ...
            if (wb.handadded != true) attachHandTo(opt, obj, wb);
            animateHand(obj, wb);
          }
        }
        if (obj.eventtype == "enteredstage") {
          if (
            obj.layer.hasClass("handadded") &&
            !obj.layer.hasClass("handremoved")
          ) {
  ...
  var lookForNextLayer = function(obj) {
    ...
    jQuery(_d._li)
      ...
      .each(function() {
        ...
        if (
          _.active != true &&
          _.whiteboard != undefined &&
          _.whiteboard["hand_function"] != "move"
        ) {
...
****************************************/
  var moveBetweenStages = function(opt, obj, wb) {
    ...
    if (
      _.whiteboard.goto_next_layer === "on" &&
      _.whiteboard.hand_function != "move" &&
      opt.c.find(".wb-between-stations").length == 0
    ) {
      ...
      if (nextobj != undefined && nextobj.obj.length > 0) {
        var hi = hand.find(".wb-hand-inner"),
          le =
            _.timeline != undefined &&
            _.timeline._labels != undefined &&
            _.timeline._labels.frame_0_end != undefined
              ? _.timeline._labels.frame_0_end
              : 0,
          s = nextobj.startat / 1000 - le,
          pos = jQuery(_._pw).position(),
          posnew = nextobj.obj.data("_pw").position(),
          wasinstaticlayer =
            hand.closest(".tp-static-layers").length > 0 ? true : false;

        hand.appendTo(opt.ul);
        hand.addClass("wb-between-stations");
        if (wasinstaticlayer) hand.css({ zIndex: 200 });
        else hand.css({ zIndex: 100 });
        wb.handEffect = "off";
        ...
        punchgs.TweenLite.fromTo(
          ...
          { top: posnew.top, left: posnew.left }
        );
...
**************************************/
  ...
  var rotateHand = function(hand_inner, obj, _) {
    ...
    ro =
      _.hand_function == "write" || _.hand_function == "draw"
        ? Math.random() * ang - ang / 2
        : 0;
  ...
  var jitterHand = function(hand_inner, obj, _) {
    ...
    if (_.jitter_direction == "horizontal") {
      ...
      _.lastwave =
        _.lastwave == _.current_y_offset + elheight * _.jitter_offset
          ? Math.random() * (elheight * _.jitter_distance) +
            _.current_y_offset +
            elheight * _.jitter_offset
          : _.current_y_offset + elheight * _.jitter_offset;
      _.jitterspeed = _.jitterspeed || 0.05;
...
**************************************/
  var attachHandTo = function(opt, obj, wb) {
    // SET DEFAULTS
    var o =
        wb.hand_function == "move"
          ? opt.whiteboard.movehand
          : opt.whiteboard.writehand,
      element = o.markup,
      _d = obj.layer.data();

    wb.hand_full_rotation = wb.hand_full_rotation || 0;
    wb.hand_origin =
      o.transform.transformX + "px " + o.transform.transformY + "px";
    ...
    punchgs.TweenLite.set(hand.find(".wb-hand-scale"), {
      scale: opt.bw,
      transformOrigin: "0% 0%"
    });
  };
    ...
    switch (wb.hand_function) {
      ...
      case "draw":
        ...
        if (_d.splittext != undefined && _d.splittext != "none") {
            ...
            tw.eventCallback(
              ...
              function(hand_inner, obj, wb) {
                ...
                if (
                  ...
                  el.html() !== undefined &&
                  el.html().length > 0 &&
                  el.html().charCodeAt(0) != 9 &&
                  el.html().charCodeAt(0) != 10
                ) {
                  var pos = el.position(),
                    pa = el.parent(),
                    papa = pa.parent(),
                    ro =
                      wb.hand_function == "write" ? Math.random() * 10 - 5 : 0,
                  ...
                  wb.rotatespeed =
                    wb.hand_angle_repeat !== undefined
                      ? parseFloat(speed) / parseFloat(wb.hand_angle_repeat)
                      : speed > 1
                      ? speed / 6
                      : speed > 0.5
                      ? speed / 6
                      : speed / 3;
                  wb.jitterspeed =
                    wb.jitter_repeat !== undefined
                      ? parseFloat(speed) / parseFloat(wb.jitter_repeat)
                      : speed > 1
                      ? speed / 6
                      : speed > 0.5
                      ? speed / 6
          ...
          wb.rotatespeed =
            wb.hand_angle_repeat !== undefined
              ? parseFloat(s) / parseFloat(wb.hand_angle_repeat)
              : s > 1
              ? s / 6
              : s > 0.5
              ? s / 6
          ...
          wb.jitterspeed =
            wb.jitter_repeat !== undefined
              ? parseFloat(s) / parseFloat(wb.jitter_repeat)
              : s > 1
              ? s / 6
              : s > 0.5
              ? s / 6
          ...
          if (wb.hand_direction == "right_to_left") {
            ...
            punchgs.TweenLite.fromTo(
              ...
              {
                x: xt,
                ease: obj.layersettings.frames[0].ease,
                onComplete: function() {
                  wb.handEffect = "off";
                }
              }
      ...
      case "move":
        ...
        hand.data(
          "outspeed",
          obj.layersettings.frames[obj.layersettings.frames.length - 1].speed /
            1000
        );
        hand.data(
          "outease",
          obj.layersettings.frames[obj.layersettings.frames.length - 1].ease
        );

        tween[0].eventCallback(
          "onUpdate",
          function(obj) {
            ...
            wb.hand_inner_animation = punchgs.TweenLite.set(hand, {
              x: pos.x,
              y: pos.y
            });
          },
...
**************************************/
  var moveoutHand = function(opt, obj, wb) {
    ...
    if (wb.hand_function != "move") {
      ...
      tl.left =
        wb.hand_type == "right"
          ? opt.c.width() - handoffset.left + _d.eow
          : 0 - handoffset.left - _d.eow;
      tl.top = opt.c.height();
    }
